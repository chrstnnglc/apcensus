#This filters the master list of APs if an AP reading is in Payatas
#This will also compute the persistence of each AP, and look for confidently gathered APs
#This will also output the info of boxes in the Payatas
...
for i in range(0,24):
    ...
    for j in range(0,33):
        ...
        if(i < 2):
            continue
        if(i == 2 and (j < 10 or j > 21)):
            continue
        if(i == 3 and (j < 9 or j > 21)):
            continue
        if(i == 4 and (j < 6 or j > 22)):
            continue
        if((i == 5 or i == 6) and (j < 5 or j > 26)):
            continue
        if(i == 7 and j > 27):
            continue
        if((i == 8 or i == 9) and j > 29):
            continue
        if(i == 10 and j > 30):
            continue
        if((i == 12 or i == 11) and j > 31):
            continue
        if(i < 19 and j < 4):
            continue
        if(i == 23 and (j < 13 or j > 19)):
            continue
        if(i == 22 and (j < 12 or j > 20)):
            continue
        if(i == 21 and (j < 11 or j > 21)):
            continue
        if(i == 20 and (j < 10 or j > 29)):
            continue
        if(i == 19 and (j < 9 or j > 30)):
            continue
        if(i == 18 and (j < 8 or j > 30)):
            continue
        if(i == 17 and (j < 7 or j > 31)):
            continue
        if((i == 16 or i == 15) and j > 31):
            continue
    ...
    for j in range(0,33):
        ...
        for ap in ap_readings:
            ...
            if ((ap.lat <= new_box.north) and (ap.lat >= new_box.south)) and ((ap.lng <= new_box.east) and (ap.lng >= new_box.west)):
                ...
                if ap not in payatas_list:
                    new_box.ap_list.append(ap)
                    payatas_list.append(ap)
        box_list.append(new_box)
...
db_list = []
i = 0			
with open('payatas_wdb.csv','rb') as csvfile:
...
with open("results/box_info.csv","wb") as csvfile:
    ...
    write_file = csv.writer(csvfile, delimiter = ',')
    write_file.writerow(["Total No. of APs", str(len(payatas_list))])
...
with open("results/payatas_list.csv","wb") as csvfile:
    write_file = csv.writer(csvfile, delimiter = ',')
...
with open("results/payatas_persist.csv","wb") as csvfile:
    write_file = csv.writer(csvfile, delimiter = ',')
    number = 0
    write_file.writerow(["Total No. of APs", str(len(payatas_list))])
    for box in box_list:
...
with open("results/payatas_confident.csv","wb") as csvfile:
    write_file = csv.writer(csvfile, delimiter = ',')
...
res_file = open("results/payatas_confident.txt","w")
for item in confident_list:
...
with open("results/traversed_payatas_wdb.csv","wb") as csvfile:
    write_file = csv.writer(csvfile, delimiter = ',')
