...
i = 0	
with open('comp3-trikexgt.csv','rb') as csvfile:
...
for ap in ap_list:
        mul_dates = ap.dates_seen.split(",")
        if int(ap.persistence) > int(ap.box_traversal):
                count1 += 1
        elif len(mul_dates) > 2:
                count1 += 1
        else:
                count2 += 1

        if int(ap.box_time) >= 60:
                count3 += 1
        else:
                count4 += 1


print "No. of persistence APs: " + str(count1)
print "No. of non-persistent APs: " + str(count2)
print "No. APs soak time >= 60: " + str(count3)
print "No. APs soak time < 60: " + str(count4)
