...
class AP:
	def __init__(self, bssid=None, ssid=None, channel=None,enc=None,manunf=None):
		self.bssid = bssid
		self.ssid = ssid
	...
	for line in reader:
		if i != 0:		##ignore first line
			...
			if bssid not in unique_bssid:
                            new_ap = AP(bssid,ssid,channel,enc,manuf)
...
with open('group_ssid.csv','wb') as csvfile:
    ...
    write_file.writerow(["BSSID","SSID","Manufacturer","Encryption","Channel"])
    for item in pldt:
        write_file.writerow([str(item.bssid),str(item.ssid),str(item.manuf),str(item.enc),str(item.channel)])
